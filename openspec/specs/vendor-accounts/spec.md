...
Vendor accounts manages vendor entity lifecycle and credential provisioning for the invoice-manager module. Vendors are created exclusively via a CLI tool, their passcodes are stored as bcrypt hashes, and all data access goes through a repository interface to keep the database implementation decoupled from consumers.
