...
PO matching implements header-level purchase order validation as a pure function, decoupled from both the PO repository and the invoice submission handlers. The function compares three fields (vendor name, amount, currency) between an invoice and a PO, returning `matched` or `unmatched`. It is designed to be independently testable with zero I/O dependencies.
