...
Invoice entity manages the core domain object of the invoice-manager module: the invoice record itself. It defines the database schema, type definitions, and repository interface for persisting and querying invoices. Key design decisions include separate `status` (workflow stage) and `match_status` (PO validation result) columns, composite uniqueness on `(vendor_id, invoice_number)`, and self-referencing FK for superseding rejected invoices.
